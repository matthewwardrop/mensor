        ...
        def wrapper(*args, **kwargs):
            ...
            warnings.warn(
                "{}() is deprecated{}{}.".format(
                    func.__name__,
                    " as of version {}".format(version_flagged)
                    if version_flagged
                    else "",
                    " and will be removed in version {}".format(version_removal)
                    if version_removal
                    else "",
                ),
...
class SequenceMap(object):
    def __init__(self, sequence=None):
        self.__values = (
            OrderedDict([(v, v) for v in sequence]) if sequence else OrderedDict()
        )
    ...
    def process(self, **opts):
        ...
        for key, value in opts.items():
            if key not in self._options or self._options[key].get("pinned"):
                ...
                params[key] = (
                    param_schema["parser"](value) if param_schema["parser"] else value
                )
        ...
        for name, schema in self._options.items():
            if name not in params:
                if schema["required"] and not schema["pinned"]:
                    raise RuntimeError(
                        "A value for parameter `{}` ({}) was not provided.".format(
                            name, schema["desc"]
                        )
                    )
                else:
